...
public class ReseauVerifier {
  ...
  public static boolean verifierReseau(Reseau reseau) {
    return analyseReseau(reseau).isEmpty();
  }
  
  /**
   * Méthode qui analyse le réseau et fait un rapport de erreurs.

   * @param reseau Reseau à analyser
   * @return Les erreurs trouvés durant l'analyse
   */
  public static List<ReseauErreur> analyseReseau(Reseau reseau) {
    List<ReseauErreur> erreurs = new ArrayList<>();
    if (!adresseValide(reseau.getAdresse())) {
      erreurs.add(ReseauErreur.ADRESSE_ERREUR);
    }
    if (!masqueValide(reseau.getMasqueSousReseau())) {
      erreurs.add(ReseauErreur.MASQUE_INVALIDE);
    }
    if (!numerosIpValides(reseau)) {
      erreurs.add(ReseauErreur.IP_HORS_PLAGE);
    }
    if (!numerosIpUniques(reseau)) {
      erreurs.add(ReseauErreur.IP_DUPLIQUEE);
    }
